...
**iOS**: 1.13.1 (5) · **Android**: 1.13.1 (41) · **Functions**: delete `validateRegistrationCode` + `lookupGameByValidationCode` callables on both projects, redeploy `createPendingRegistration` + `confirmRegistrationPayment` with rewritten confirmation email · **Firestore rules**: unchanged (the `/eventRegistrations` collection stays admin-SDK-only) · **Web**: full redeploy (staging + prod) — `.well-known/apple-app-site-association` + `.well-known/assetlinks.json` deleted, `firebase.json` hosting headers entry for those paths removed
