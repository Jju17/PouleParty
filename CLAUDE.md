...
- **Hunters**: Join via the 6-character game code. Must physically find the chicken and enter the `foundCode` to win. When the game is linked to a paid event batch (`Game.registrationBatchId != null`, PP-52), the JoinFlow inserts a manual-entry step for the unique registration code received by email, validated + single-use-claimed server-side via the `validateRegistrationCode` callable (no deeplink — see PP-52 flow). Free games (no `registrationBatchId`) join with the gameCode alone.
