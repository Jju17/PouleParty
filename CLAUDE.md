...
### Roles
Membership is a single server-authoritative map (`roles`, PP-107 — see "Player roles" below): a uid maps to exactly one of `chicken` / `hunter` / `gameMaster`. The derived client accessors (`chickenId`, `hunterIds`, `gameMasterIds`, `isChicken(uid)`, ...) keep the read-site vocabulary below valid even though only the one map is stored.

- **Creator** (admin owner): Creates the game, owns it (`creatorId`), can delete or reconfigure it while `status == waiting`. By default also starts as the chicken (the create payload seeds `roles == { <creatorId>: 'chicken' }`) but a GameMaster can re-designate someone else (PP-26). `creatorId` is ownership and is orthogonal to the play role; the creator always also carries a role in `roles`.
- **Chicken**: The single player who runs and hides (the uid whose role is `chicken`). Shows the 4-digit `foundCode` to hunters who physically find them. Distinct from the creator: a GameMaster or the creator can swap the chicken to any current hunter while `status == waiting`, through the `designateChicken` callable (atomic swap — the demoted chicken becomes a hunter, so a uid always keeps exactly one role). Use `game.isChicken(userId)` for the "is this user the chicken" check.
- **Hunters**: Join via the 6-character game code through the `joinGame` callable (the server writes the hunter role, the `/players/{uid}` team-name doc, and the membership index in one transaction). Must physically find the chicken and enter the `foundCode` to win. When the game is linked to a paid event batch (`Game.registrationBatchId != null`, PP-52), the JoinFlow inserts a manual-entry step for the unique registration code received by email, validated + single-use-claimed server-side via the `validateRegistrationCode` callable (no deeplink — see PP-52 flow) BEFORE `joinGame` runs. Free games (no `registrationBatchId`) join with the gameCode alone.
- **GameMasters** (PP-23, PP-24, PP-70): 0..N validators distinct from the chicken and the hunters. The creator sets a 4-digit `gameMasterPassword` at game creation; anyone with the code calls `joinAsGameMaster`, which sets their role to `gameMaster` (rejecting the chicken, an existing hunter, the creator, and a `done` game — MED-13). Validators get a dedicated observer map (`GameMasterMapFeature` on iOS, `GameMasterMapScreen` on Android) that streams the chicken's position, every hunter, and every spawned power-up in read-only mode. They never write their own GPS and cannot collect / activate power-ups. Hunters always broadcast their position when at least one GameMaster has joined (`game.gameMasterIds.isNotEmpty()`), regardless of `chickenCanSeeHunters` or `gameMode`, so the GM sees them even in `stayInTheZone`. The password lives in `/games/{gameId}/private/` (admin-SDK-only); only the `roles` map is on the public Game doc.

### Player roles (PP-107)
Game membership is one server-authoritative map on the game doc, `roles: { <uid>: "chicken" | "hunter" | "gameMaster" }`. It replaced the three separate fields (`chickenId` string + `hunterIds` array + `gameMasterIds` array). Because a uid maps to exactly one role, a "ghost" (member with no role) or a double-role is impossible by construction.

- **Server-authoritative**: `firestore.rules` only allows the create payload to seed `roles == { <creatorId>: 'chicken' }`; every later change is denied to clients. All mutations go through callables in `functions/src/roles.ts` (admin SDK, bypassing rules):
  - `joinGame(gameId, teamName)` — caller joins as a hunter; writes the role, the `/players/{uid}` team-name doc, and the `/users/{uid}/memberships/{gameId}` reverse index in one transaction. Replaces the old client `hunterIds` arrayUnion + client registration write.
  - `designateChicken(gameId, newChickenUid)` — creator/GM only, `status == waiting`, `newChickenUid` must currently be a hunter. Atomic swap: the new chicken leaves the hunter pool, the demoted chicken becomes a hunter (closes HIGH-3, the loose designation rule, and LOW-16, the designate-vs-LAUNCH race).
  - `leaveGame(gameId)` — hunters and GameMasters only (the chicken cancels the game instead); clears the role, the team-name doc, and the membership index.
  - `joinAsGameMaster` (in `gameMaster.ts`) sets `roles[uid] = 'gameMaster'` (excludes the chicken / existing hunter / creator, rejects `status == 'done'` — MED-13).
- **Client accessors**: iOS `Models/Game+Computed.swift` and Android `model/Game.kt` expose COMPUTED `chickenId` / `hunterIds` / `gameMasterIds` / `role(of:)` / `isChicken` / `isHunter` / `isGameMaster`, all derived from `roles`. Read-site code (`game.gameMasterIds.isNotEmpty()`, `game.isChicken(uid)`) is unchanged.
- **Runtime role re-route**: when `roles` changes mid-`waiting` (a chicken swap), both clients re-route the affected players to the correct map (chicken ↔ hunter) and show a one-time "new chicken" alert. Previously a swapped player was stranded on the wrong map.
...
3. **In-app linking** : when the admin creates the matching Game in-app (admin mode `jujurahier`, lifted maxPlayers), `Game.registrationBatchId` is set to the same `batchId` the form was using (e.g. `game-06-06-2026`). Currently set **by hand in Firestore** on the D-Day game (no wizard field). The Game doc is otherwise standard.
4. **Mobile JoinFlow — single-use code gate (manual entry, no deeplink)** : when a hunter resolves a gameCode whose `Game.registrationBatchId != null`, the iOS `JoinFlowFeature` / Android `HomeViewModel` inserts a `validationCodeEntry` / `ValidationCodeEntry` step between `codeValidated` and `joiningWithTeamName`. The hunter **types the unique code from their registration email**; it's validated server-side by the `validateRegistrationCode` callable (`ApiClient.validateRegistrationCode` / `FirestoreRepository.validateRegistrationCode`). The callable runs a transaction: query `eventRegistrations where batchId == X && code == Y && paid == true`, then **single-use-claim** it (`claimedBy`/`claimedAt`) — the first device to validate a code claims it, any later attempt with a different UID gets `alreadyUsed`. Returns a discriminated `{ status: valid | invalid | alreadyUsed }`, never PII (rules lock `/eventRegistrations` reads to `if false`). Wrong/used code → inline error, stay on the step. Per-UID rate limit (`/validationRateLimits`, 10 / 10 min / 1 h lock) throttles brute force. Games **without** `registrationBatchId` skip this step entirely (join with the gameCode alone, unchanged). This is the anti-share mechanism: 1 paid registration = 1 join. No auth/account is involved — the anonymous UID is just the claim token.

   **Deliberately NOT present** (removed for Apple App Store 3.1.1 compliance, twice-rejected when present): no Universal Link / App Link deeplink, no `pouleparty.be/join` CTA, no `associated-domains` entitlement, no `.well-known/{apple-app-site-association,assetlinks.json}`, no `lookupGameByValidationCode` callable, no `DeeplinkBus`. The code is entered manually; the confirmation email shows it as text (it's also the wristband-pickup token at the start bar). The binary contains no link to the paid web form.
...
/games/{gameId}
  ...
  ├── foundCode, status (one of `waiting` / `readyToLaunch` (PP-71) / `inProgress` / `done`), winners, creatorId  (the chicken `lastHeartbeat` ping moved to RTDB presence, PP-102)
  ├── roles: { <uid>: "chicken" | "hunter" | "gameMaster" }  (PP-107 single source of truth for membership — replaced `chickenId` / `hunterIds` / `gameMasterIds`. A uid has exactly one role. Seeded at create to `{ <creatorId>: 'chicken' }`, then written ONLY by the role callables in `functions/src/roles.ts` + `joinAsGameMaster`; clients read it through the derived `chickenId` / `hunterIds` / `gameMasterIds` accessors and never write it. `creatorId` stays as ownership and also appears in `roles`.)
  ...
  ├── /players/{userId}            → One doc per in-app hunter (`{ teamName, joinedAt }`). PP-107 renamed it from `/registrations` and made it server-write-only: written exclusively by the role callables (`joinGame` upserts the joining hunter; `designateChicken` moves the demoted chicken in / drops the new chicken out), clients only READ it. Any participant can read every doc so the GameMaster can pick a chicken from the team-name list (PP-86) and the challenges leaderboard can show team names. Deliberately distinct from the top-level `/eventRegistrations` collection (PP-52, paying web registrations) — different namespace and ownership.
  ...
  ├── /aggregates/leaderboard      → PP-103 denormalized leaderboard read-model: `{ entries: { <hunterId>: { teamName, totalPoints } } }`. Written only by `validateChallengeSubmission` / `applyOutOfZonePenalty` (admin SDK, same transaction as the completion). Clients read this ONE doc for the live challenge leaderboard instead of streaming the whole challengeCompletions collection (kills the N-doc fan-out). Rules: read for participants, `write: false`.
...
Anonymous Firebase Auth. Users don't create accounts — a UID is generated on first launch and persists. The creator's UID is stored as `creatorId`; the play roles (chicken / hunter / gameMaster) are identified by the UID's entry in the `roles` map (PP-107). User profiles (nickname, FCM token, platform) are saved to `/users/{userId}`.
...
- `/games/{gameId}/meta` → `{ creatorId, gameMode, status, roles: { <uid>: role } }` (PP-107). Membership mirror written by the `mirrorGameMetaToRtdb` Cloud Function so the RTDB rules (`database.rules.json`) can authorize (RTDB rules can't read Firestore; they read `meta.roles.<uid>` and compare it to `'chicken'` / `'hunter'` / `'gameMaster'`). The same function removes the whole `/games/{gameId}` RTDB subtree on `status == done` / deletion (cleanup).
...
See `firestore.rules`. Key principle: the creator has full control over their game; membership (`roles`) is **server-authoritative** (PP-107) — clients can never write it. The create payload may only seed `roles == { <creatorId>: 'chicken' }`; every later change (join, leave, GM join, chicken swap) goes through a callable (admin SDK, bypassing rules). Hunters' only remaining client writes are `winners` (now also callable-only since CRIT-3) and `powerUps` collection. Subcollection writes are restricted by role; `/players` and `/users/{uid}/memberships` are read-only to clients (callable-written). **Power-up creation is denied to all clients** (`allow create: if false`) — only the Cloud Function (which bypasses rules via admin SDK) can spawn them. Power-up collection uses transactions to prevent double-collection.
