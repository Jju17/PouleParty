...
/challenges/{challengeId}          → Global, dev-managed challenge catalog. `{title, body, points, lastUpdated, type: "oneShot" | "repeatable" (PP-27), location: GeoPoint? (bar partner pin), proximityRadiusMeters: Int? (consumed by PP-72 anti-cheat, default 100 m set by migration), partner: String? (brand label)}`. `oneShot` = validable once per hunter (street stunts), `repeatable` = validable multiple times (bar partners). The PP-31 `migrateChallengesV2` Cloud Function backfills `type: "oneShot"` and `proximityRadiusMeters: 100` on every legacy doc.
